...
private ["_F_Uncache", "_nameString", "_units", "_indexes", "_array", "_side", "_group", "_cachedData", "_unit", "_addUnits", "_unitsGrouped"];
...
ZEN_STD_Parse_GetArgumentDefault(_addUnits, 1, [])
_addUnits = [_addUnits] call Zen_ConvertToObjectArray;

_cachedData = [];
while {count _addUnits > 0} do {
    _unit = _addUnits select 0;
    _unitsGrouped = (units group _unit) arrayIntersect _addUnits;
    {
        0 = [_addUnits, _x] call Zen_ArrayRemoveValue;
...
if ([_nameString] call Zen_IsCached) then {
    {
        if ((count _x > 0) && {((_x select 0) isKindOf "MAN")}) then {
            ...
            if (({side _x == _side} count allGroups) <= 143) then {
...

                {
                    ...
                    // _waypoint setWaypointCompletionRadius (_x select 2),
                    ...
                    // _waypoint setWaypointPosition (_x select 6),
                ...
                ZEN_FMW_Code_Error("Zen_Uncache", "Uncaching all units would exceed the maximum number of groups.")
            };
        };
    } forEach _units;
...
0 = [(_array select 2), _cachedData] call Zen_ArrayAppendNested;
